...
clean_data <- read.csv("data/overdose.csv") %>% 
    clean_names() %>%
   select(-x) %>%
  rename(date = dat_format, time = dat_format_1) %>%
...
#07/14 had the most - 23 - overdose calls. There are no days without overdose calls, 06/11 had only one call. 
```{r}
clean_data %>%
...
#Weekends have the highest number of reports, followed by Friday. 
```{r}
...
#4540 SILVER SPRING RD has the most, 36 calls. 
```{r}

locations <- clean_data %>%
  group_by(loc) %>%
  summarise(count = n()) %>%
  arrange(desc(count))
...
A4.
#The previous question and responses made me curious to explore the neighborhood around 4540 Silver Spring Rd. Is this a single house, or is it the same building where the 36 calls were made? How safe is the area for living, and what are the local crime rates? Zillow shows that a house at this address was sold last year for less than half a million dollars. Additionally, how does the neighborhood's safety impact property values?
