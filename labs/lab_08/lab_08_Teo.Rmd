...
```{r}
pgeorges_2022_overdoses <- read_csv("data/prince_georges_2022_overdoses.csv")
...
pgeorges_2022_overdoses <- prince_georges_2022_overdoses %>%
...
pgeorges_2022_overdoses %>%
 ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
  coord_flip() + 
  theme_economist() +
  labs(
    title="Prince George Total Overdose Calls for 2022",
    x = "month",
    y = "total calls",
...
```{r}
week_overdoses <- prince_georges_2022_overdoses %>%
...
week_overdoses |>
 ...
 theme_economist() +
  labs(
    title="Prince George Total Weekly Overdose Calls for 2022",
    x = "Week",
    y = "Total calls",
    caption = "source: Baltimore County"
...
zip_code_calls <- pgeorges_2022_overdoses %>%
  group_by(zipcode) %>%
...
overdose_joined <- zip_code_calls |> left_join(md_zipcodes, by=c('zipcode'='GEOID'))
...
overdose_joined %>%
  ...
  labs(
    title = "Overdose 911 Calls vs Median Income by ZIP Code",
    x = "Total Calls",
