...
#remove links from the website: 1. find the link 2.  replace
```
...
#select text and asking to give unique words
```
...
```{r}
unique_words |>
  count(word, sort = TRUE) |> #sort stands for the sorting and presenting the data in a graph
  top_n(25) |>
...
#Distance is more important between the words than the value of the words

```{r}
unique_words |>
  anti_join(stop_words) |> #unique words and stop words will be eliminated
  group_by(word) |>
  tally(sort=TRUE) |>
...
unique_words_2022 |>
  anti_join(stop_words) |>
  group_by(word) |>
  tally(sort=TRUE) |> #tidyverse verb, suggests summarize
  mutate(percent = (n/sum(n))*100) |>
  top_n(10)
```
...
alerts |>
  ...
  filter(!word1 %in% stop_words$word) |> #checking if there is any stopwords, and if so, then eliminate
