...
#umd_courses is the name of data, and the next line is grouping the terms. The last part of this code is saying to count the number of classes and call is count_classses. 
...
#Now we have results that shows the highest number first and going to the lowest. In the previous codeblock it was showing the ascending order, which means that the results were from the lowest to the highest. 
...
**Answer**
#We are looking now how many courses each department offered for each semester. We now have departments' information included that was not there in the previous outcome.
...
#Now we are getting a response for the question how many classes and how many seats were offered for each term. total_seats is a new thing here, and it summarises all the seats available for the term.
...
#Now, along with the total seats for each term, the mean and median seat counts are also displayed. The top terms have very similar total, median, and mean seat numbers.
...
#Now, along with the total, mean, and median seat counts for each term, the smallest and largest seat numbers are also displayed. The data is sorted in descending order by total seats.
...
1. Which class is the most popular?  

2. Which classes have disappeared due to a lack of student interest?  

3. Which professor's name appears most frequently?
