...
#arrange - puts the data in a chronologocal order
```{r}
md_grants_loans |> 
...
#here we changed the name
#str_to_upper - is using to make everything uppercased
...
#There is 58 rows that are duplictaed. Some of these have just one common word and it thinks it is a duplicate. 
```
...
#distinct helps to get ride of dupes
#we as
...
**Task** Write and run code to answer our original question: 
- which zip code has gotten the most amount of money from the Maryland Tourism Board? 
- Where is that zip code, and does that zip code make sense to you as the top recipient? 
- Write a sentence describing the top result. 

**Answer**
#ZIP code 21202 received the most amount of grant $8938468.15. The zip code itself does not tell much, besides saying that it is from a state of Maryland. As I checked it in cleaned_md_grants_loans that zip code belongs to several grantees, such as DEPARTMENT OF BUSINESS AND ECONOMIC DEVELOPMENT - MARYLAND TOURISM BOARD, MARYLAND ENVIRONMENTAL SERVICES/RESTORATION, DEPARTMENT OF LABOR, LICENSING AND REGULATION, etc. 

```{r}

tourism_grants <- cleaned_md_grants_loans |>
