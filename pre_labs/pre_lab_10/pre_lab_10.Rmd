...
```{r}
md_zips <- st_read("data/cb_2018/BNDY_ZIPCodes11Digit_MDP.shp")
...
```{r}
md_county_notices <- read_csv("data/Maryland_Foreclosure_Data_by_County.csv") |> slice(1) |> pivot_longer(cols=-c('Date', 'Type'), names_to='county', values_to = 'notices') #read noteces by county
...
#mutate county and turn it into each word in title capital letter, remove ", Maryland" at the end
  
#str_to_title - each word in capital letter
#side="both" - to do this on both sides(remove)
#side="left" - remove only on the left side
